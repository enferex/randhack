...
static void print_rand_values(const char *prefix, int n_values)
{
    int i;

    printf(">> Printing %d values from random() and rand()...\n", n_values);
    for (i=0; i<n_values; i++) 
      printf("%s %ld %d\n", prefix, random(), rand());
}


/* Fork a process to show that its rand() has also been comprimised */
...
int main(void)
{
    ...
    print_rand_values("main", 10);

    /* How large 'randtbl' can vary.
     * See glibc-2.7 source.
     *
